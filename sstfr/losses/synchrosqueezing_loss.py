...
Implements Eq. (6) of the SSTFR paper:
    ...
    L_SSA = sum_{t,d} |h_t^(d)|^2 * (omega_SSM(t,d) - omega_SST(t, k(d)))^2
            ---------------------------------------------------------------
                              sum_{t,d} |h_t^(d)|^2 + eps
...
def assign_ridges_to_channels(
    ...
    # |D x K| pairwise distances (in circular metric)
    # Broadcasting: (D, 1) vs (1, K) -> (D, K)
    diff = (channel_omegas.unsqueeze(1) - ridge_omegas.unsqueeze(0))
    distance = diff.abs()  # simple L1 in rad/s; ridges don't wrap in practice
...
class SynchrosqueezingAlignmentLoss(nn.Module):
    ...
    Usage:
        ...
        loss = loss_fn(H, omega_ssm_target, channel_omegas, ridge_omegas)

    The SST target (`omega_ssm_target`) is precomputed offline and passed in
    per batch. See scripts/precompute_sst.py for the offline pipeline.
    ...
    def forward(
        ...
        Two calling conventions for omega_sst_target:
        ...
        B) Ridge-indexed: shape (B, L, K). Requires `ridge_omegas` of shape (K,).
           ...
           and gather target[..., k(d)] for each channel.
        ...
        # 2. Resolve the target IF tensor to shape (B, L, D)
        if omega_sst_target.shape == (B, L, D):
            target = omega_sst_target
        else:
            if ridge_omegas is None:
                raise ValueError(
                    "ridge_omegas required when target shape is (B, L, K) rather than (B, L, D)"
                )
            assignment = assign_ridges_to_channels(channel_omegas, ridge_omegas)  # (D,)
            # Gather along the last dim: for each d, pick target[..., assignment[d]]
            # target: (B, L, K) -> (B, L, D)
        ...
        # (no previous sample), so we pad it with zero. Including it in the loss
        # would punish the model for a mathematically meaningless position.
        # The mask is built as a boolean tensor and applied multiplicatively.
        time_mask = torch.ones_like(weights)
        time_mask[:, 0, :] = 0.0
        weights = weights * time_mask

        # 6. Amplitude-weighted mean
